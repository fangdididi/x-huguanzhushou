...
- 新增单任务运行锁，多个 twitter/x.com 标签页或窗口同时打开时，只允许一个页面恢复或执行任务，避免重复关注、重复评论或重复检测。
- 运行中的任务会定期续租锁；页面刷新、跳转后仍由原任务继续接管，其他页面只显示日志提示，不会抢占执行。
- 任务停止、完成或异常结束后会释放运行锁，关闭页面后锁会自动过期，降低意外中断后的手动清理成本。
